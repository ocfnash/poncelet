...
class PonceletGif extends Component {
  ...
  generateSingleFrame(canvas, ellipsesPath2D, phi) {
    const {a, b, r, n, SIZE, SCALE, CENTER} = this.props;
    ...
    ctx.strokeStyle = 'red';
    ctx.stroke(new Path2D(svgPathFromPts(applyAffineTrans(getPonceletPoints(a, b, r, n, phi), CENTER, SCALE))));
    return canvas.toDataURL('image/png');
  }
  generateFrames() {
    const {a, b, r, SCALE, CENTER} = this.props;
  ...
  renderGif() {
    ...
    return (<img id="animated-poncelet-gif" width={imgSize} height={imgSize} src={imgSrc} />);
  }
  render() {
    const SIZE = this.props.SIZE;
